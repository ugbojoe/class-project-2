...
it('should return a 200 response and return the new movie', done => {
 api.post('/')
 .set('Accept', 'application/json')
 .send({ title: 'Air Force One', genre: 'action' })
 .expect(200)
 .end((err, res) => {
   const movie = res.body;
   expect(movie).to.not.equal(null);
   expect(movie).to.have.property('title').that.equals('Air Force One');
   done();
  });
});
});
